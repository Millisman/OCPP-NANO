#include <string.h>
#include "sha1.h"
#include "mcu/mcu.h"

#define HMAC_IPAD 0x36
#define HMAC_OPAD 0x5c
#define SHA1_K0  0x5a827999
#define SHA1_K20 0x6ed9eba1
#define SHA1_K40 0x8f1bbcdc
#define SHA1_K60 0xca62c1d6

namespace ocpp {

void Sha1Class::set_m(sha1_mem *m) {
    mem = m;
    memset(mem, 0, sizeof(sha1_mem));
}


void Sha1Class::init() {
    memcpy_P(mem->state.b, sha1InitState, HASH_LENGTH);
    byteCount = 0;
    bufferOffset = 0;
}

uint32_t Sha1Class::rol32(uint32_t number, uint8_t bits) {
    return ((number << bits) | (number >> (32-bits)));
}

void Sha1Class::hashBlock() {
    uint8_t i;
    uint32_t a,b,c,d,e,t;
    
    a=mem->state.w[0];
    b=mem->state.w[1];
    c=mem->state.w[2];
    d=mem->state.w[3];
    e=mem->state.w[4];
    for (i=0; i<80; i++) {
        if (i>=16) {
            t = mem->buffer.w[(i+13)&15] ^ mem->buffer.w[(i+8)&15] ^ mem->buffer.w[(i+2)&15] ^ mem->buffer.w[i&15];
            mem->buffer.w[i&15] = rol32(t,1);
        }
        if (i<20) {
            t = (d ^ (b & (c ^ d))) + SHA1_K0;
        } else if (i<40) {
            t = (b ^ c ^ d) + SHA1_K20;
        } else if (i<60) {
            t = ((b & c) | (d & (b | c))) + SHA1_K40;
        } else {
            t = (b ^ c ^ d) + SHA1_K60;
        }
        t+=rol32(a,5) + e + mem->buffer.w[i&15];
        e=d;
        d=c;
        c=rol32(b,30);
        b=a;
        a=t;
    }
    mem->state.w[0] += a;
    mem->state.w[1] += b;
    mem->state.w[2] += c;
    mem->state.w[3] += d;
    mem->state.w[4] += e;
}

void Sha1Class::addUncounted(uint8_t data) {
    mem->buffer.b[bufferOffset ^ 3] = data;
    bufferOffset++;
    if (bufferOffset == BLOCK_LENGTH) {
        hashBlock();
        bufferOffset = 0;
    }
}

size_t Sha1Class::write(uint8_t data) {
    ++byteCount;
    addUncounted(data);
    
    return sizeof(data);
}

void Sha1Class::pad() {
    // Implement SHA-1 padding (fips180-2 §5.1.1)
    
    // Pad with 0x80 followed by 0x00 until the end of the block
    addUncounted(0x80);
    while (bufferOffset != 56) addUncounted(0x00);
    
    // Append length in the last 8 bytes
    addUncounted(0); // We're only using 32 bit lengths
    addUncounted(0); // But SHA-1 supports 64 bit lengths
    addUncounted(0); // So zero pad the top bits
    addUncounted(byteCount >> 29); // Shifting to multiply by 8
    addUncounted(byteCount >> 21); // as SHA-1 supports bitstreams as well as
    addUncounted(byteCount >> 13); // byte.
    addUncounted(byteCount >> 5);
    addUncounted(byteCount << 3);
}


uint8_t* Sha1Class::result(void) {
    // Pad to complete the last block
    pad();
    
    // Swap byte order back
    for (int i=0; i<5; i++) {
        uint32_t a,b;
        a=mem->state.w[i];
        b=a<<24;
        b|=(a<<8) & 0x00ff0000;
        b|=(a>>8) & 0x0000ff00;
        b|=a>>24;
        mem->state.w[i]=b;
    }
    
    // Return pointer to hash (20 characters)
    return mem->state.b;
}



void Sha1Class::initHmac(const uint8_t* key, int keyLength) {
    uint8_t i;
    memset(mem->keyBuffer,0,BLOCK_LENGTH);
    if (keyLength > BLOCK_LENGTH) {
        // Hash long keys
        init();
        for (;keyLength--;) write(*key++);
        memcpy(mem->keyBuffer,result(),HASH_LENGTH);
    } else {
        // Block length keys are used as is
        memcpy(mem->keyBuffer,key,keyLength);
    }
    // Start inner hash
    init();
    for (i=0; i<BLOCK_LENGTH; i++) {
        write(mem->keyBuffer[i] ^ HMAC_IPAD);
    }
}

uint8_t* Sha1Class::resultHmac(void) {
    uint8_t i;
    // Complete inner hash
    memcpy(mem->innerHash,result(),HASH_LENGTH);
    // Calculate outer hash
    init();
    for (i=0; i<BLOCK_LENGTH; i++) write(mem->keyBuffer[i] ^ HMAC_OPAD);
    for (i=0; i<HASH_LENGTH; i++) write(mem->innerHash[i]);
    return result();
}


}
