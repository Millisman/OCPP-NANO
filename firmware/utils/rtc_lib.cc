...
void DateTime::timestamp(char *buffer25, timestampOpt opt) {
    // large enough for any DateTime, including invalid ones

  // Generate timestamp according to opt
  switch (opt) {
  case TIMESTAMP_TIME:
    // Only time
      sprintf(buffer25, "%02d:%02d:%02d", hh, mm, ss);
    break;
  case TIMESTAMP_DATE:
    // Only date
      sprintf(buffer25, "%u-%02d-%02d", 2000U + yOff, m, d);
    break;
  default:
    // Full
      sprintf(buffer25, "%u-%02d-%02dT%02d:%02d:%02d", 2000U + yOff, m, d, hh, mm,
            ss);
  }
}
...
uint32_t DateTime::unixtime(void) const {
    uint32_t t;
    t = time2long(date2days(yOff, m, d), hh, mm, ss);
    t += SECONDS_FROM_1970_TO_2000;  // seconds from 1970 to 2000
    return t;
}
...
uint32_t div1000(uint32_t a) {
    return (a/1000);
}


rtc_unix_t EEMEM rtc_unix_eep;

// bool RTC_Millis::Save_EEP() {
//     unix_base.crc8 = gencrc((uint8_t*)&unix_base, sizeof(rtc_unix_t)-1);
//     eeprom_write_block(&unix_base, &rtc_unix_eep, sizeof(rtc_unix_t));
//     LOG_P(PSTR("RTC_Millis Save_EEP CRC=%u\r\n"), unix_base.crc8);
//     return false;
// }

bool RTC_Millis::Load_Unix() { // return true if error 
    eeprom_read_block(&unix_base, &rtc_unix_eep, sizeof(rtc_unix_t));
    uint8_t crc = gencrc8((uint8_t*)&unix_base, sizeof(rtc_unix_t)-1);

    if (crc != unix_base.crc8) {
        unix_base = {
            .unixtime = 1631297707
        };
        return true;
...
void RTC_Millis::Init() {
Load_Unix();
m_cur_ms = m_last_ms = mcu::Timer::millis();
}




void RTC_Millis::SetDateTime(uint8_t y,uint8_t m,uint8_t d,uint8_t h,uint8_t mn,uint8_t s) {
    adjust(DateTime(y,m,d,h,mn,s));
}


void RTC_Millis::adjust(const DateTime& dt) {
    unix_base.unixtime = dt.unixtime() - div1000(mcu::Timer::millis());
}

// period 1 sec
void RTC_Millis::Update() {
    m_cur_ms = mcu::Timer::millis();
    if (m_cur_ms < m_last_ms) { // overflow 49.71day
        unix_base.unixtime += 4294967;
        unix_base.crc8 = gencrc8((uint8_t*)&unix_base, sizeof(rtc_unix_t)-1);
        eeprom_write_block(&unix_base, &rtc_unix_eep, sizeof(rtc_unix_t));
    }
    
    now = (uint32_t)(unix_base.unixtime + div1000(m_cur_ms));
    m_last_ms = m_cur_ms;
}

bool RTC_Millis::Save_EEP() {
    m_cur_ms = mcu::Timer::millis();
    if (m_cur_ms > m_last_ms) { // no overflow
        uint32_t ubase = unix_base.unixtime;
        unix_base.unixtime = ubase +  div1000(m_cur_ms);//  m_cur_ms / 1000;
        unix_base.crc8 = gencrc8((uint8_t*)&unix_base, sizeof(rtc_unix_t)-1);
        eeprom_write_block(&unix_base, &rtc_unix_eep, sizeof(rtc_unix_t));
        unix_base.unixtime = ubase;
    }
    m_last_ms = m_cur_ms;
    return false;
}*/
