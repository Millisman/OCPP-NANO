...
{    
    ...
    usbser << stream::PGM << PSTR("Unix time init:") << settings.get_UnixTime() << EOL;
...
void ManTime::onEvent(const event::Event& event) {
    if (event.id == events::UPDATE) {
        if (lastCalc) {
            if (lastUnix != settings.get_UnixTime()) {
                lastUnix = settings.get_UnixTime();
                event::Loop::post(event::Event(events::TICK_1_SEC));
                save_interval--;
                if (save_interval==0) {
                    save_interval = SAVE_INTERVAL;
                    settings.save_UnixTime();
                    usbser << EOL << stream::PGM << PSTR("UT saved:") << settings.get_UnixTime() << EOL;
                }
            }
            lastCalc = false;
        } else {
